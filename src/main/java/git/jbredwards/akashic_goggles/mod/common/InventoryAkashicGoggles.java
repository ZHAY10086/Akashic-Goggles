...
{
    @Nonnull
    public static final Object2IntMap<Item> SUPPORTED_GOGGLES = new Object2IntOpenHashMap<>();
    static { SUPPORTED_GOGGLES.defaultReturnValue(-2); }

    @Nonnull
    public static final ResourceLocation SLOT_TEXTURE = new ResourceLocation(Tags.MOD_ID, "textures/gui/slot.png");
    public static final int WIDTH = 4, HEIGHT = 2, SLOT_SIZE = 18;
    ...
    static void renderTooltipSlots(@Nonnull final RenderTooltipEvent.PostText event) {
        if(event.getStack().getItem() instanceof ItemAkashicGoggles && event.getStack().hasCapability(IDropInItem.DROP_IN_CAPABILITY, null)) {
            ...
            if(inventory != null) {
                ...
                final int xOffset = (event.getWidth() >> 1) - (SLOT_SIZE * Math.min(slots, WIDTH) >> 1);
                final int yOffset = SLOT_SIZE *- (int)Math.ceil(slots / (double)WIDTH) - 12;

                for(int slot = 0; slot < slots; slot++) {
                    final int x = event.getX() + SLOT_SIZE * (slot % WIDTH) + xOffset;
                    final int y = event.getY() + SLOT_SIZE * (slot / WIDTH % HEIGHT) + yOffset;
                    drawSlot(SLOT_TEXTURE, event.getFontRenderer(), inventory.getStackInSlot(slot), x, y);
                }
            }
        }
    }
    ...
    public boolean isItemValid(final int slot, @Nonnull final ItemStack stack) {
        ...
        return SUPPORTED_GOGGLES.getInt(stack.getItem()) == targetDamage && stacks.stream().noneMatch(stack::isItemEqualIgnoreDurability);
    }
